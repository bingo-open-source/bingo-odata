...
public class TestODataRequestRoute {
	...
	public void testCustomRegex(){
		...
		ODataRequestRoute route = ODataRequestRoute.compile("*", "/{name : [a]+}", new DummyHandler());
		
		assertFalse(route.matches("GET", "/bba", params));
		assertTrue(route.matches("GET", "/aaa", params));
		assertEquals("aaa", params.get("name"));
	}
