...
import odata.ORelatedEntitiesLinkInline;
import odata.ORelatedEntityLink;
import odata.Throwables;
import odata.edm.EdmEntitySet;
import odata.producer.inmemory.BeanModel;
...
public class InternalUtil {
    ...
    public static String reflectionToString(final Object obj) {
        ...
        String content = EnumerableImpl.of(objClass.getFields()).select(new Func1<Field, String>() {
            ...
            for (OLink l : oe.getLinks()) {
                if (l instanceof ORelatedEntitiesLinkInline) {
                    ...
                    if (beanModel.canWrite(ol.getTitle())) {
                        Collection<Object> relatedEntities = ol.getRelatedEntities() == null ? null : EnumerableImpl.of(ol.getRelatedEntities()).select(new Func1<OEntity, Object>() {
    ...
    public static String getEntityRelId(List<String> keyPropertyNames, final List<OProperty<?>> entityProperties, String entitySetName) {
        ...
        if (keyPropertyNames != null) {
            Object[] keyProperties = EnumerableImpl.of(keyPropertyNames).select(new Func1<String, OProperty<?>>() {
