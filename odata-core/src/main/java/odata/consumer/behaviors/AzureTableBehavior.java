...
import odata.consumer.ODataConsumer;
...
public class AzureTableBehavior implements OClientBehavior {
    ...
    public ODataClientRequest transform(ODataClientRequest request) {
        try {
            ...
            request = request.header("x-ms-version", "2009-09-19").header("x-ms-date", date).header("Authorization", auth).header("DataServiceVersion", "1.0;NetFx").header(
                    "MaxDataServiceVersion",
                        "1.0;NetFx");

            if (isPut || (isDelete && !isTableRequest) || (isPost && request.getHeaders().containsKey(ODataConstants.Headers.X_HTTP_METHOD)))
