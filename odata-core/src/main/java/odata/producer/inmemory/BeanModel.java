...
public class BeanModel {
    ...
    public Iterable<?> getCollectionValue(Object target, String collectionName) {
        ...
        try {
            ...
            if (obj == null)
                ...
                return obj.getClass().isArray() ? IterableEnumerable.of((Object[]) obj) : (Iterable<?>) obj;
        } catch (Exception e) {
            throw Throwables.propagate(e);
        }
    }
        ...
        try {
            ...
            if (collectionValue != null) {
                ...
                if (List.class.isAssignableFrom(clazz)) {
                    value = collectionValue instanceof List ? (List<T>) collectionValue : new ArrayList<T>(collectionValue);
                } else if (Set.class.isAssignableFrom(clazz)) {
                    value = collectionValue instanceof Set ? (Set<T>) collectionValue : new HashSet<T>(collectionValue);
                } else
            ...
            if (getterType != null) {
                ...
                if (getterType != null && !getterType.equals(setterType))
                    throw new RuntimeException(String.format(
                            "Inconsistent types for property %s.%s: getter type %s, setter type %s",
    ...
    private static Map<String, Method> getBeanGetters(Class<?> clazz) {
        ...
        for (Method method : clazz.getMethods()) {
            ...
            if (methodName.startsWith("get") && methodName.length() > 3 && Character.isUpperCase(methodName.charAt(3)) && method.getParameterTypes().length == 0
                    && !method.getReturnType().equals(Void.TYPE) && !Modifier.isStatic(method.getModifiers())) {
                String name = methodName.substring(3);
                rt.put(name, method);
            }
            if (methodName.startsWith("is") && methodName.length() > 2 && Character.isUpperCase(methodName.charAt(2)) && method.getParameterTypes().length == 0
                    && (method.getReturnType().equals(Boolean.class) || method.getReturnType().equals(Boolean.TYPE)) && !Modifier.isStatic(method.getModifiers())) {
