...
public class EdmEntityType extends EdmStructuralType {
    ...
    private EdmEntityType(String namespace, String alias, String name, Boolean hasStream, ImmutableList<String> keys, EdmEntityType baseType, List<EdmProperty.Builder> properties,
            ImmutableList<EdmNavigationProperty> navigationProperties, EdmDocumentation doc, ImmutableList<EdmAnnotation<?>> annotations, Boolean isAbstract) {
        super(baseType, namespace, name, properties, doc, annotations, isAbstract);
        this.alias = alias;
        this.hasStream = hasStream;

        this.keys = (keys == null || keys.isEmpty()) ? (baseType == null ? findConventionalKeys() : null) : keys;
    ...
    private List<String> findConventionalKeys() {
        for (EdmProperty prop : getProperties()) {
            if (prop.getName().equalsIgnoreCase("Id") && prop.getType().isSimple() && !prop.isNullable()) {
                IterableEnumerable.of(prop.getName()).toList();
            }
        }
        return null;
    }
     ...
     */
    public List<EdmNavigationProperty> getDeclaredNavigationProperties() {
        return CollectionUtils.newList(navigationProperties);
    }
    ...
    public EdmNavigationProperty findDeclaredNavigationProperty(String name) {
        return IterableEnumerable.of(navigationProperties).firstOrNull(OPredicates.nameEquals(EdmNavigationProperty.class, name));
    }
    ...
    public List<EdmNavigationProperty> getNavigationProperties() {
        return isRootType() ? getDeclaredNavigationProperties() : Collections.concat(getBaseType().getNavigationProperties(),getDeclaredNavigationProperties());
    }
    ...
    public static class Builder extends EdmStructuralType.Builder<EdmEntityType, Builder> implements Named {
        ...
        public EdmEntityType buildImpl() {
            ...
            return new EdmEntityType(
                    namespace,
                        ...
                        properties,
                        ImmutableList.copyOf(builtNavProps),
                        getDocumentation(),
        ...
        public String getFQBaseTypeName() {
            return baseTypeNameFQ != null ? baseTypeNameFQ : (baseType != null ? baseType.getFullyQualifiedTypeName() : null);
        }
