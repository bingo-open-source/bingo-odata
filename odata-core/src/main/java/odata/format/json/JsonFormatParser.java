...
import odata.format.json.JsonFeedFormatParser.JsonEntry;
import odata.format.json.JsonFeedFormatParser.JsonFeed;
    ...
    protected JsonObjectPropertyValue getValue(JsonEvent event, EdmEntitySet ees, String name, JsonStreamReader jsr, JsonEntry entry) {
        ...
        if (DEFERRED_PROPERTY.equals(event.asStartProperty().getName())) {
            ...
            if (navProp != null) {
                ...
                rt.entities = IterableEnumerable.of(feed.getEntries()).cast(JsonEntry.class).select(new Func1<JsonEntry, OEntity>() {
