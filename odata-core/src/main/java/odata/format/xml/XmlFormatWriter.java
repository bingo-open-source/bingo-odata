...
import odata.ORelatedEntityLinkInline;
import odata.OSimpleObject;
...
public class XmlFormatWriter {
    ...
    protected String writeEntry(XMLWriter2 writer, OEntity oe, List<OProperty<?>> entityProperties, List<OLink> entityLinks, String baseUri, String updated, EdmEntitySet ees, boolean isResponse) {
            ...
            if (isResponse) {
                ...
                for (OLink link : entityLinks) {
                    ...
                    if (link.isInline()) {
                        ...
                        // deferred link.
                        writeElement(writer, "link", null, "rel", rel, "type", type, "title", link.getTitle(), "href", href);
                    }
                }
            } else {
        ...
        // oe is null for creates
                    "term",
                    oe == null ? ees.getType().getFullyQualifiedTypeName() : oe.getEntityType().getFullyQualifiedTypeName(),
                    "scheme",
                    scheme);
        ...
        if (linkToInline instanceof ORelatedEntitiesLinkInline) {
            ...
            if (entities != null && !entities.isEmpty()) {
                ...
                for (OEntity entity : ((ORelatedEntitiesLinkInline) linkToInline).getRelatedEntities()) {
                    ...
                    writeEntry(writer, entity, entity.getProperties(), entity.getLinks(), baseUri, updated, entity.getEntitySet(), isResponse);
