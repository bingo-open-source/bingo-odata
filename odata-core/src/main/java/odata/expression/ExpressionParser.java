...
public class ExpressionParser {
    ...
    private static Set<String> METHODS = IteratedEnumerable.of(
                                               Methods.CAST,
                                                   Methods.ISOF,
                                                   Methods.ENDSWITH,
                                                   Methods.STARTSWITH,
    ...
    public static List<OrderByExpression> parseOrderBy(String value) {
        ...
        if (DUMP_EXPRESSION_INFO) {
            dump(value, tokens, IteratedEnumerable.of(expressions).toArray(CommonExpression.class));
        }

        return IteratedEnumerable.of(expressions).select(new Func1<CommonExpression, OrderByExpression>() {
    ...
    private static void dump(String value, List<Token> tokens, CommonExpression... expressions) {
        String msg = "[" + value + "] -> " + IteratedEnumerable.of(tokens).join("");
        if (expressions != null) {
            msg = msg + " -> " + IteratedEnumerable.of(expressions).select(new Func1<CommonExpression, String>() {
    ...
    public static List<EntitySimpleProperty> parseExpand(String value) {
        ...
        return IteratedEnumerable.of(expressions).select(new Func1<CommonExpression, EntitySimpleProperty>() {
    ...
    private static class ExpressionToken extends Token {
        ...
        @Override
        public String toString() {
            return IteratedEnumerable.of(tokens).join("");
        }
    }
}
