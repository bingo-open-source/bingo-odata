...
public class InMemoryEdmGenerator implements EdmGenerator {
    ...
    public EdmDataServices.Builder generateEdm(EdmDecorator decorator) {
        ...
        Map<String, EdmEntityType.Builder> entityTypesByName = Enumerator.create(entityTypes).toMap(new Func1<EdmEntityType.Builder, String>() {
        ...
        Map<String, EdmEntitySet.Builder> entitySetByName = Enumerator.create(entitySets).toMap(new Func1<EdmEntitySet.Builder, String>() {
            public String evaluate(EdmEntitySet.Builder input) {
                return input.getName();
            }
        });
        ...
        EdmEntityContainer.Builder container = EdmEntityContainer.newBuilder().setName(containerName).setIsDefault(true).addEntitySets(entitySets).addAssociationSets(associationSets);

        containers.add(container);

        EdmSchema.Builder schema = EdmSchema.newBuilder().setNamespace(namespace).addEntityTypes(entityTypes).addAssociations(associations).addEntityContainers(containers).addComplexTypes(
                edmComplexTypes);
    ...
    private void createComplexTypes(EdmDecorator decorator, List<EdmComplexType.Builder> complexTypes) {
        for (String complexTypeName : complexTypeInfo.keySet()) {
            ...
            EdmComplexType.Builder typeBuilder = EdmComplexType.newBuilder().setNamespace(namespace).setName(typeInfo.typeName).addProperties(properties);
    ...
    private void createStructuralEntities(EdmDecorator decorator, List<EdmEntitySet.Builder> entitySets, List<EdmEntityType.Builder> entityTypes) {
        ...
        for (String entitySetName : eis.keySet()) {
            ...
            EdmEntityType.Builder eet = EdmEntityType
                    .newBuilder()
    ...
    private void createNavigationProperties(List<EdmAssociation.Builder> associations, List<EdmAssociationSet.Builder> associationSets, Map<String, EdmEntityType.Builder> entityTypesByName,
            Map<String, EdmEntitySet.Builder> entitySetByName, Map<Class<?>, String> entityNameByClass) {
    ...
    private void generateToOneNavProperties(List<EdmAssociation.Builder> associations, List<EdmAssociationSet.Builder> associationSets, Map<String, EdmEntityType.Builder> entityTypesByName,
            Map<String, EdmEntitySet.Builder> entitySetByName, Map<Class<?>, String> entityNameByClass, String entityTypeName, InMemoryEntityInfo<?> ei) {
    ...
    private void generateToManyNavProperties(List<EdmAssociation.Builder> associations, List<EdmAssociationSet.Builder> associationSets, Map<String, EdmEntityType.Builder> entityTypesByName,
            Map<String, EdmEntitySet.Builder> entitySetByName, Map<Class<?>, String> entityNameByClass, String entityTypeName, InMemoryEntityInfo<?> ei, Class<?> clazz1) {
        ...
        for (String assocProp : ei.properties.getCollectionNames()) {
            ...
            try {
                EdmAssociation.Builder assoc = Enumerator.create(associations).firstOrNull(new Predicate<EdmAssociation.Builder>() {

                    public boolean evaluate(EdmAssociation.Builder input) {
    ...
    private Collection<EdmProperty.Builder> toEdmProperties(EdmDecorator decorator, PropertyModel model, String[] keys, String structuralTypename) {
        ...
        Set<String> keySet = Enumerator.create(keys).toSet();
