...
import bingo.odata.OComplexObjects;
import bingo.odata.OEntities;
...
import bingo.odata.edm.EdmItem;
...
import bingo.odata.producer.ExpressionEvaluator;
import bingo.odata.producer.ODataProducer;
import bingo.odata.producer.PropertyPath;
import bingo.odata.producer.PropertyPathHelper;
import bingo.odata.producer.QueryInfo;
import bingo.odata.producer.Responses;
import bingo.odata.producer.ExpressionEvaluator.VariableResolver;
 ...
 * InMemoryProducer and the bingo.odata.edm pojos together are sufficient to
...
public class MetadataProducer implements ODataProducer {
    ...
    public EntitiesResponse getEntities(String entitySetName, QueryInfo queryInfo) {
        ...
        return Responses.entities(c.entities, c.entitySet, null, // inline count
                    null); // skip token.
    }
    ...
    protected OEntity getSchema(Context c, EdmSchema schema) {
        ...
        return OEntities.create(c.entitySet, OEntityKey.create(Edm.Schema.Namespace, schema.getNamespace()), // OEntityKey entityKey,
                    props,
                    links);
    }
    ...
    private OEntity getStructuralType(Context c, EdmStructuralType st) {
        ...
        if (c.pathHelper.isSelected(Edm.StructuralType.NavProps.Properties)) {
            if (c.pathHelper.isExpanded(Edm.StructuralType.NavProps.Properties)) {
                ...
                List<OEntity> properties = new ArrayList<OEntity>(st.getDeclaredProperties().size());
        ...
        return OEntities.create(c.entitySet, OEntityKey.create(Edm.StructuralType.Namespace, st.getNamespace(), Edm.StructuralType.Name, st.getName()), // OEntityKey entityKey,
                    props,
                    links);
    }
            ...
            for (NamespacedAnnotation<?> a : item.getAnnotations()) {
                if (a.getValue() != null) {
                    ...
                    if (c.pathHelper.isSelected(propName)) {
                        Object override = this.decorator != null ? this.decorator.getAnnotationValueOverride(item, a, c.flatten, c.locale, c.queryInfo == null ? null : c.queryInfo.customOptions)
                                : null;
    ...
    private OEntity getProperty(EdmStructuralType queryType, EdmStructuralType et, EdmProperty p, Context c) {
        ...
        return OEntities.create(entitySet, OEntityKey.create(Edm.Property.Namespace, et.getNamespace(), Edm.Property.EntityTypeName, et.getName(), Edm.Property.Name, p.getName()), props, Collections
                .<OLink> emptyList());
    }
    ...
    public EntityResponse getEntity(String entitySetName, OEntityKey entityKey, EntityQueryInfo queryInfo) {
        ...
        if (entitySetName.equals(Edm.EntitySets.Schemas)) {
            findSchema(c);
        } else if (entitySetName.equals(Edm.EntitySets.EntityTypes) || entitySetName.equals(Edm.EntitySets.RootEntityTypes)) {
            findStructuralType(c, true, entitySetName.equals(Edm.EntitySets.RootEntityTypes));
        } else if (entitySetName.equals(Edm.EntitySets.ComplexTypes) || entitySetName.equals(Edm.EntitySets.RootComplexTypes)) {
