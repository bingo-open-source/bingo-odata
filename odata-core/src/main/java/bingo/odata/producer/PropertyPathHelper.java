...
public class PropertyPathHelper {
    ...
    public PropertyPathHelper(List<EntitySimpleProperty> select, List<EntitySimpleProperty> expand, String selectR, String expandR) {
        setup(select, expand, selectR != null && selectR.length() > 0 ? ExpressionParser.parseExpand(selectR) : null, expandR != null && expandR.length() > 0 ? ExpressionParser.parseExpand(expandR)
                : null);
    }

    private void setup(String select, String expand, String selectR, String expandR) {
        setup(select != null && select.length() > 0 ? ExpressionParser.parseExpand(select) : null, expand != null && expand.length() > 0 ? ExpressionParser.parseExpand(expand) : null, selectR != null
                && selectR.length() > 0 ? ExpressionParser.parseExpand(selectR) : null, expandR != null && expandR.length() > 0 ? ExpressionParser.parseExpand(expandR) : null);
    }
    ...
    public boolean isSelected(String propName) {
        ...
        if (this.isSelectionLimitedRecursive()) {
            ...
            for (PropertyPath p : this.selectRPaths) {
                ...
                // matches current of:
                //     .../blat
                if (p.getLastComponent().equals(propName) && this.currentNavPath.getNComponents() >= 1 && this.currentNavPath.getLastComponent().equals(p.getFirstComponent())) {
                    return true;
                }
            }
        }
