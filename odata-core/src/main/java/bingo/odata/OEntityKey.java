...
import bingo.odata.zinternal.lang.NamedValues;
...
public class OEntityKey {
    ...
    public Set<NamedValue<?>> asComplexValue() {
        assertComplex();
        return (Set<NamedValue<?>>) (Object) toSortedSet(Arrays.cast(values, new NamedValue[values.length]), OComparators.namedValueByNameRaw());
    }
    ...
    private static <T> SortedSet<T> toSortedSet(Collection<T> values, Comparator<T> comparator) {
        TreeSet<T> rt = new TreeSet<T>(comparator);
