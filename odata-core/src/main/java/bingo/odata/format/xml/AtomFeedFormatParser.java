...
import bingo.odata.stax2.XMLEventWriter2;
...
public class AtomFeedFormatParser extends XmlFormatParser implements FormatParser<Feed> {
    ...
    AtomFeed parseFeed(XMLEventReader2 reader) {
        ...
        feed.entries = Enumerator.create(rt).cast(Entry.class);
        ...
        while (reader.hasNext()) {
            ...
            if (event.isStartElement() && event.asStartElement().getName().getNamespaceUri().equals(NS_DATASERVICES)) {
                ...
                if (et != null && (!et.isSimple())) {
                    op = OProperties.complex(name, (EdmComplexType) et, isNull ? null : Enumerator.create(parseProperties(reader, event.asStartElement(), metadata)).toList());
                } else {
    ...
    private DataServicesAtomEntry parseDSAtomEntry(String etag, XMLEventReader2 reader, XMLEvent2 event) {
        ...
        dsae.etag = etag;
        dsae.properties = Enumerator.create(parseProperties(reader, event.asStartElement(), this.metadata)).toList();
        return dsae;
    }
    ...
    private OEntity entityFromAtomEntry(EdmDataServices metadata, EdmEntitySet entitySet, DataServicesAtomEntry dsae, FeedCustomizationMapping mapping) {
        ...
        if (mapping != null) {
            Enumerator<OProperty<?>> properties = Enumerator.create(dsae.properties);
                ...
                if (link.type.equals(XmlFormatWriter.atom_feed_content_type)) {
                    ...
                    if (link.inlineContentExpected) {
                        ...
                        if (link.inlineFeed != null && link.inlineFeed.entries != null) {
                            ...
                            relatedEntities = Enumerator.create(link.inlineFeed.entries).cast(DataServicesAtomEntry.class).select(new Func1<DataServicesAtomEntry, OEntity>() {
                                public OEntity evaluate(DataServicesAtomEntry input) {
