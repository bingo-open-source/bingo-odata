...
import bingo.odata.edm.EdmFunctionParameter;
...
import bingo.odata.edm.EdmFunctionParameter.Mode;
...
public class EdmxFormatParser extends XmlFormatParser {
    ...
    private void resolve() {
        ...
        final Map<String, EdmEntityType.Builder> allEetsByFQName = Enumerator.create(dataServices.getEntityTypes()).toMap(new Func1<EdmEntityType.Builder, String>() {
            public String evaluate(EdmEntityType.Builder input) {
                return input.getFQAliasName() != null ? input.getFQAliasName() : input.getFullyQualifiedTypeName();
            }
        });
        final Map<String, EdmAssociation.Builder> allEasByFQName = Enumerator.create(dataServices.getAssociations()).toMap(new Func1<EdmAssociation.Builder, String>() {
            public String evaluate(EdmAssociation.Builder input) {
        ...
        for (EdmSchema.Builder edmSchema : dataServices.getSchemas()) {
            ...
            for (EdmEntityType.Builder eet : edmSchema.getEntityTypes()) {
                ...
                for (int i = 0; i < navProps.size(); i++) {
                    ...
                    List<EdmAssociationEnd.Builder> finalEnds = Enumerator.create(tmp.getFromRoleName(), tmp.getToRoleName()).select(new Func1<String, EdmAssociationEnd.Builder>() {
                        public EdmAssociationEnd.Builder evaluate(String input) {
            ...
            for (final EdmEntityContainer.Builder edmEntityContainer : edmSchema.getEntityContainers()) {
                for (int i = 0; i < edmEntityContainer.getAssociationSets().size(); i++) {
                    ...
                    List<EdmAssociationSetEnd.Builder> finalEnds = Enumerator.create(tmpEas.getEnd1(), tmpEas.getEnd2()).select(
                            new Func1<EdmAssociationSetEnd.Builder, EdmAssociationSetEnd.Builder>() {
                                public EdmAssociationSetEnd.Builder evaluate(final EdmAssociationSetEnd.Builder input) {

                                    EdmAssociationEnd.Builder eae = ea.getEnd1().getRole().equals(input.getRoleName()) ? ea.getEnd1() : ea.getEnd2().getRole().equals(input.getRoleName()) ? ea
                                            .getEnd2() : null;

                                    if (eae == null)
                                        throw new IllegalArgumentException("Invalid role name " + input.getRoleName());

                                    EdmEntitySet.Builder ees = Enumerator
                                            .create(edmEntityContainer.getEntitySets())
            ...
            for (final EdmEntityContainer.Builder edmEntityContainer : edmSchema.getEntityContainers()) {
                for (int i = 0; i < edmEntityContainer.getFunctionImports().size(); i++) {
                    ...
                    EdmEntitySet.Builder ees = Enumerator.create(edmEntityContainer.getEntitySets()).firstOrNull(new Predicate<EdmEntitySet.Builder>() {
                        public boolean evaluate(EdmEntitySet.Builder input) {
                    ...
                    edmEntityContainer.getFunctionImports().set(
                            i,
                                EdmFunctionImport.newBuilder().setName(tmpEfi.getName()).setEntitySet(ees).setReturnType(typeBuilder).setHttpMethod(tmpEfi.getHttpMethod()).addParameters(
                                        tmpEfi.getParameters()));
                }
            }
    ...
    private EdmEntityContainer.Builder parseEdmEntityContainer(XMLEventReader2 reader, String schemaNamespace, StartElement2 entityContainerElement) {
        ...
        while (reader.hasNext()) {
            ...
            if (isStartElement(event, EDM2006_ENTITYSET, EDM2007_ENTITYSET, EDM2008_1_ENTITYSET, EDM2008_9_ENTITYSET, EDM2009_ENTITYSET))
                edmEntitySets.add(EdmEntitySet.newBuilder().setName(getAttributeValueIfExists(event.asStartElement(), "Name")).setEntityTypeName(
                        getAttributeValueIfExists(event.asStartElement(), "EntityType")));
            ...
            if (isEndElement(event, entityContainerElement.getName())) {
                return EdmEntityContainer.newBuilder().setName(name).setIsDefault(isDefault).setLazyLoadingEnabled(lazyLoadingEnabled).addEntitySets(edmEntitySets).addAssociationSets(
                        edmAssociationSets).addFunctionImports(edmFunctionImports);
            }
        }
        throw new UnsupportedOperationException();
    ...
    private EdmFunctionImport.Builder parseEdmFunctionImport(XMLEventReader2 reader, String schemaNamespace, StartElement2 functionImportElement) {
        ...
        while (reader.hasNext()) {
            ...
            if (isEndElement(event, functionImportElement.getName())) {
                return EdmFunctionImport.newBuilder().setName(name).setEntitySetName(entitySet).setReturnTypeName(returnType).setIsCollection(isCollection).setHttpMethod(httpMethod).addParameters(
                        parameters);
            }
        }
        throw new UnsupportedOperationException();
    ...
    private EdmAssociationSet.Builder parseEdmAssociationSet(XMLEventReader2 reader, String schemaNamespace, StartElement2 associationSetElement) {
        ...
        while (reader.hasNext()) {
            ...
            if (isStartElement(event, EDM2006_END, EDM2007_END, EDM2008_1_END, EDM2008_9_END, EDM2009_END))
                ends.add(EdmAssociationSetEnd.newBuilder().setRoleName(event.asStartElement().getAttributeByName("Role").getValue()).setEntitySetName(
                        event.asStartElement().getAttributeByName("EntitySet").getValue()));
