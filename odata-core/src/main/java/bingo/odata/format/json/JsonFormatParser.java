...
import bingo.odata.format.json.JsonFeedFormatParser.JsonEntry;
import bingo.odata.format.json.JsonFeedFormatParser.JsonFeed;
...
public class JsonFormatParser {
    ...
    protected JsonEntry parseEntry(JsonEntryMetaData jemd, EdmEntitySet ees, JsonStreamReader jsr) {
        ...
        entry.oentity = toOEntity(ees, entry.getEntityType(), entry.getEntityKey(), entry.properties, links);
        return entry;
    }
    ...
    private OEntity toOEntity(EdmEntitySet entitySet, EdmEntityType entityType, OEntityKey key, List<OProperty<?>> properties, List<OLink> links) {
    ...
    protected JsonEntryMetaData parseMetadata(JsonStreamReader jsr) {
        ...
        while (jsr.hasNext()) {
            ...
            } else if (event.isStartProperty() && TYPE_PROPERTY.equals(event.asStartProperty().getName())) {
                ensureEndProperty(event = jsr.nextEvent());
                jemd.type = event.asEndProperty().getValue();
            } else if (event.isStartProperty() && ETAG_PROPERTY.equals(event.asStartProperty().getName())) {
                ensureEndProperty(event = jsr.nextEvent());
        ...
        if (DEFERRED_PROPERTY.equals(event.asStartProperty().getName())) {
            ...
            if (navProp != null) {
                ...
                if (eprop == null) {
                    ...
                    EdmComplexType ct = (eprop.getType() instanceof EdmComplexType) ? ((EdmComplexType) eprop.getType()) : metadata.findEdmComplexType(eprop.getType().getFullyQualifiedTypeName());
